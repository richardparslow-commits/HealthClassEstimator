...
CARRIER_RULES.amam = {
    ...
    eligibility: {
      products: "Express Term (level term to 95: 10/20/25/30-yr periods, + Return of Premium), Term Made Simple (level term to 95: 10/15/20/30-yr); Dignity Solutions final-expense whole life is a separate lane (ages 50-85)",
      ...
      chartNote: "Express Term and Term Made Simple share the same simplified-issue build chart (minimum weight, Table-2 and Table-4 maximums) and the same Preferred build chart (unisex). Dignity Solutions uses its own three-plan build chart (Immediate / Graded / Return of Premium)."
    },
...
CARRIER_RULES.corebridge = {
  ...
  eligibility: {
    ...
    chartNote: "Build chart (SimpliNow Legacy Max / level column): minimum and maximum weight per height. Outside the range — decline."
  },
  nicotine: {
    classes: [
  ...
  build: {
    ...
    rules: {
      ...
      lowBuildReview: "Below the chart minimum for height — decline.",
      aboveStandard: "Above the chart maximum for height — decline.",
      note: "SimpliNow Legacy Max (level benefit) build chart: minimum and maximum weight per height. Outside the range — decline. The graded (SimpliNow Legacy) column is more permissive; the Level column is the best offer."
    }
  },
  bp: {
    preferred_plus: { sys: 160, dia: 100 },
